...
def test_add_should_also_fail():
    assert(utils.add5(5) == 15)

def test_add_should_also_fail_as_well():          #if this functions is uncommented, this unit test will also be ran.
    assert(utils.add5(5) == 15)

# a random comment, change and save this line (or make any other change) while running the main file
# so that the CI will run unit tests. ffdsafds